...
def extract_HCEST_params(path):
    ...
    df_final = df_final.drop(columns=['param' + str(i+1) for i in range(len(df_total))[1:]])
#     df_final = df_final.set_index(list(df_final)[0])

    df_final.to_csv(path + '/' + path.split('/')[-2] + '_fitparm.csv', index=False)
...
def residual_HCEST2(path, assignment, ol_num, name=None, ylim = [-0.12, 0.12], xticks = [-6, 6, 7]):
    '''
